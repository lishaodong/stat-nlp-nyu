...
public class ChartParser {
	...
	static class Lexicon {
		...
		public Lexicon(BufferedReader in) throws IOException {
			...
			while (in.ready()) {
				...
				if (!matcher.matches())
					throw new RuntimeException("Bad line in lexicon: " + line);
	...
	static class Grammar {
		...
		public Grammar(BufferedReader in) throws IOException {
			...
			while (in.ready()) {
				...
				Matcher matcher = linePattern.matcher(line);
				if (!matcher.matches())
					throw new RuntimeException("Bad line in grammar: " + line);
				...
				if (childArray.length == 1) {
					...
					// binary rule
					addBinary(new BinaryRule(parent, childArray[0],
							childArray[1]));
				} else {
					// left-trie binarize an n-ary rule
					StringBuilder intermediateSymbol = new StringBuilder(
							"@NP->");
					...
					while (pos < childArray.length) {
						...
						if (pos == 1)
							leftChild = childArray[0];
						String rightChild = childArray[pos];
						intermediateSymbol.append("_");
						intermediateSymbol.append(rightChild);
						String intermediateParent = intermediateSymbol
								.toString();
						if (pos == childArray.length - 1)
							intermediateParent = parent;
						addBinary(new BinaryRule(intermediateParent, leftChild,
								rightChild));
						pos++;
					}
				}
	...
	static class Edge {
		...
		public List<Tree<String>> getTrees() {
			...
			for (Backtrace backtrace : backtraces) {
				if (backtrace instanceof WordBacktrace) {
					...
					trees.add(new Tree<String>(getLabel(), Collections
							.singletonList(new Tree<String>(wordBacktrace
									.getWord()))));
				} else if (backtrace instanceof UnaryBacktrace) {
					UnaryBacktrace unaryBacktrace = (UnaryBacktrace) backtrace;
					List<Tree<String>> childTrees = unaryBacktrace
							.getChildEdge().getTrees();
					for (Tree<String> childTree : childTrees) {
						List<Tree<String>> children = Collections
								.singletonList(childTree);
					...
					List<Tree<String>> leftTrees = binaryBacktrace
							.getLeftEdge().getTrees();
					List<Tree<String>> rightTrees = binaryBacktrace
							.getRightEdge().getTrees();
		...
		public boolean equals(Object o) {
			...
			if (start != edge.start)
				return false;
			if (label != null ? !label.equals(edge.label) : edge.label != null)
				return false;
	...
	static class Chart {
		static class Index {
			...
			public boolean equals(Object o) {
				...
				if (pos != index.pos)
					return false;
				if (label != null ? !label.equals(index.label)
						: index.label != null)
					return false;
		...
		public void addEdge(Edge edge) {
			CollectionUtils.addToValueList(edgesByLeftIndex,
					makeLeftIndex(edge), edge);
			CollectionUtils.addToValueList(edgesByRightIndex,
					makeRightIndex(edge), edge);
		}
		...
		public List<Edge> getEdgesByRightIndex(String label, int end) {
			return CollectionUtils.getValueList(edgesByRightIndex, new Index(
					label, end));
		}

		public List<Edge> getEdgesByLeftIndex(String label, int start) {
			return CollectionUtils.getValueList(edgesByLeftIndex, new Index(
					label, start));
		}
	}
	...
	private void initialize() {
		...
		for (int wordI = 0; wordI < sentence.size(); wordI++) {
			...
			for (String tag : tagList) {
				...
				if (verbose)
					System.err.println("Adding tagging of " + tagEdge + " for "
							+ word);
		...
		for (int wordI = 0; wordI <= sentence.size(); wordI++) {
			...
			if (verbose)
				System.err.println("Adding empty edge of " + emptyEdge);
	...
	private void processEdge(Edge edge) {
		// project unaries
		for (UnaryRule unaryRule : grammar
				.getUnaryRulesByChild(edge.getLabel())) {
			Edge resultEdge = makeEdge(unaryRule.getParent(), edge.getStart(),
					edge.getEnd());
			if (verbose)
				System.err.println("Using unary " + unaryRule + " on " + edge
						+ " to create " + resultEdge);
			discoverEdge(resultEdge);
			resultEdge.addBacktrace(edge);
		}
		// project left siblings
		for (BinaryRule binaryRule : grammar.getBinaryRulesByRightChild(edge
				.getLabel())) {
			List<Edge> leftMatches = chart.getEdgesByRightIndex(
					binaryRule.getLeftChild(), edge.getStart());
			for (Edge leftMatch : leftMatches) {
				Edge resultEdge = makeEdge(binaryRule.getParent(),
						leftMatch.getStart(), edge.getEnd());
				if (verbose)
					System.err.println("Using binary " + binaryRule + " on "
							+ leftMatch + " and " + edge + " to create "
							+ resultEdge);
				discoverEdge(resultEdge);
		...
		for (BinaryRule binaryRule : grammar.getBinaryRulesByLeftChild(edge
				.getLabel())) {
			List<Edge> rightMatches = chart.getEdgesByLeftIndex(
					binaryRule.getRightChild(), edge.getEnd());
			for (Edge rightMatch : rightMatches) {
				Edge resultEdge = makeEdge(binaryRule.getParent(),
						edge.getStart(), rightMatch.getEnd());
				if (verbose)
					System.err.println("Using binary " + binaryRule + " on "
							+ edge + " and " + rightMatch + " to create "
							+ resultEdge);
				discoverEdge(resultEdge);
	...
	private void discoverEdge(Edge edge) {
		if (edge.isDiscovered())
			return;
		if (verbose)
			System.err.println("Discovering edge " + edge);
		edge.setDiscovered();
		agenda.addLast(edge);
	}
	...
	public void parse(List<String> sentence) {
		...
		while (!agenda.isEmpty()) {
			...
			if (verbose)
				System.err.println("Popped edge: " + edge);
			processEdge(edge);
			chart.addEdge(edge);
			if (verbose)
				System.err.println("Added to chart: " + edge);
		}
	}
	...
	public static void main(String[] args) throws Exception {
		if (args.length != 3 && args.length != 4) {
			System.err
					.println("usage: java nlp.classical.ChartParser [-verbose] lexiconFileName grammarFileName \"sentence to parse\"");
			System.exit(0);
		}
		...
		Lexicon lexicon = new Lexicon(new BufferedReader(new FileReader(
				lexiconFileName)));
		Grammar grammar = new Grammar(new BufferedReader(new FileReader(
				grammarFileName)));
