...
public class WordAlignmentTester {
  ...
  private static void getData(
    ...
    int[] maxes = new int[]{1000, 2000, 10000};
    double[] buckets = new double[]{0.05, 0.1, 0.2};
    for (double ratio: ratios) {
      for (int max: maxes) {
        for (double bucket: buckets) {
          System.out.println("ratio: " + ratio + ", max: " + max + ", bucket:" + bucket);

          trainingSentencePairs = readSentencePairs(basePath + "/training",
            max);
          Model2WordAlinger aligner = new Model2WordAlinger(trainingSentencePairs, ratio, bucket);
          test(aligner, testSentencePairs, testAlignments, false);
        }
      }
    }
  }
  ...
  private static void test(WordAligner wordAligner, List<SentencePair> testSentencePairs, Map<Integer, Alignment> testAlignments, boolean verbose) {
    ...
    System.out.println("Iteration: " + ((Model1WordAlinger) wordAligner).numIter);
