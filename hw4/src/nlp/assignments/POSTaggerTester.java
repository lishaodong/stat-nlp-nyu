...
public class POSTaggerTester {
  ...
  static class TaggedSentence {
    ...
    public boolean equals(Object o) {
      ...
      if (tags != null ? !tags.equals(taggedSentence.tags) : taggedSentence
          .tags != null)
        return false;
  ...
  static class State {
    ...
    private static transient Interner<State> stateInterner = new
        Interner<State>(new Interner.CanonicalFactory<State>() {
    ...
    public static State buildState(String previousPreviousTag, String
        previousTag, int position) {
    ...
    public boolean equals(Object o) {
      ...
      if (previousPreviousTag != null ? !previousPreviousTag.equals(state
          .previousPreviousTag) : state.previousPreviousTag != null)
        return false;
    ...
    private void setState(String previousPreviousTag, String previousTag, int
        position) {
    ...
    private State(State state) {
      setState(state.getPreviousPreviousTag(), state.getPreviousTag(), state
          .getPosition());
    }
  }
  ...
  static class POSTagger {
    ...
    public void train(List<TaggedSentence> taggedSentences) {
      localTrigramScorer.train(extractLabeledLocalTrigramContexts
          (taggedSentences));
    }
    ...
    public void validate(List<TaggedSentence> taggedSentences) {
      localTrigramScorer.validate(extractLabeledLocalTrigramContexts
          (taggedSentences));
    }

    private List<LabeledLocalTrigramContext>
    extractLabeledLocalTrigramContexts(List<TaggedSentence> taggedSentences) {
      List<LabeledLocalTrigramContext> localTrigramContexts = new
          ArrayList<LabeledLocalTrigramContext>();
      for (TaggedSentence taggedSentence : taggedSentences) {
        localTrigramContexts.addAll(extractLabeledLocalTrigramContexts
            (taggedSentence));
      }
    ...
    extractLabeledLocalTrigramContexts(TaggedSentence taggedSentence) {
      List<LabeledLocalTrigramContext> labeledLocalTrigramContexts = new
          ArrayList<LabeledLocalTrigramContext>();
      List<String> words = new BoundedList<String>(taggedSentence.getWords(),
          START_WORD, STOP_WORD);
      List<String> tags = new BoundedList<String>(taggedSentence.getTags(),
          START_TAG, STOP_TAG);
      for (int position = 0; position <= taggedSentence.size() + 1;
           position++) {
        labeledLocalTrigramContexts.add(new LabeledLocalTrigramContext(words,
            position, tags.get(position - 2), tags.get(position - 1), tags
            .get(position)));
      }
      ...
      for (int position = 0; position <= sentence.size() + 1; position++) {
        ...
        for (State state : states) {
          ...
          LocalTrigramContext localTrigramContext = new LocalTrigramContext
              (sentence, position, state.getPreviousPreviousTag(), state
                  .getPreviousTag());
          Counter<String> tagScores = localTrigramScorer.getLogScoreCounter
              (localTrigramContext);
  ...
  static class LocalTrigramContext {
    ...
    public LocalTrigramContext(List<String> words, int position, String
        previousPreviousTag, String previousTag) {
      this.words = words;
      this.position = position;
      this.previousTag = previousTag;
      this.previousPreviousTag = previousPreviousTag;
  ...
  static class LabeledLocalTrigramContext extends LocalTrigramContext {
    ...
    public LabeledLocalTrigramContext(List<String> words, int position,
                                      String previousPreviousTag, String
                                          previousTag, String currentTag) {
  ...
  static class MostFrequentTagScorer implements LocalTrigramScorer {
    ...
    CounterMap<String, String> preToTags = new CounterMap<String, String>();
    CounterMap<String, String> prepreToTags = new CounterMap<String, String>();
    Counter<String> unknownWordTags = new Counter<String>();
    Counter<String> unknownWords = new Counter<String>();
    ...
    public Counter<String> getLogScoreCounter(LocalTrigramContext
                                                  localTrigramContext) {
      ...
      Set<String> allowedFollowingTags = allowedFollowingTags(tagCounter
          .keySet(), localTrigramContext.getPreviousPreviousTag(),
          localTrigramContext.getPreviousTag());

      String bigram = makeBigramString(localTrigramContext
          .getPreviousPreviousTag(), localTrigramContext.getPreviousTag());
      Counter<String> prepreToTagsCounter = prepreToTags.getCounter(bigram);
      Counter<String> preToTagsCounter = preToTags.getCounter
          (localTrigramContext.getPreviousTag());

      Counter<String> logScoreCounter = new Counter<String>();
      Counter<String> UNKProbCounter = classifier.getProbabilities(word);
      for (String tag : tagToWords.keySet()) {
        double logScore;
        if (isUnknown) {
          if (word.equals("\"")) {
            if (tag.equals("``") || tag.equals("''")) {
              logScore = Math.log(0.5);
            } else {
              logScore = Double.NEGATIVE_INFINITY;
            }
          } else {
            logScore = Math.log(0.9 * UNKProbCounter.getCount(tag) + 0.1 * unknownWordTags.getCount(tag));
          }
        } else {
          logScore = Math.log(tagToWords.getCounter(tag).getCount(word));
        }
        logScore += Math.log(0.6 * prepreToTagsCounter.getCount(tag) +
            0.3 * preToTagsCounter.getCount(tag) + 0.1 * tags.getCount(tag));
        if (!restrictTrigrams ||
      ...
      if (logScoreCounter.size() == 0) {
        System.err.println("stop");
      }
      return logScoreCounter;
    }

    private Set<String> allowedFollowingTags(Set<String> tags, String
        previousPreviousTag, String previousTag) {
    ...
    private String makeTrigramString(String previousPreviousTag, String
        previousTag, String currentTag) {
      return previousPreviousTag + " " + previousTag + " " + currentTag;
    }

    private String makeBigramString(String previousPreviousTag, String
        previousTag) {
    ...
    public void train(List<LabeledLocalTrigramContext>
      ...
      for (LabeledLocalTrigramContext labeledLocalTrigramContext :
        ...
        if (!tagToWords.keySet().contains(tag)) {
          // word is currently unknown, so tally its tag in the
          // unknown tag counter
          unknownWords.incrementCount(tag, 1.0);
        }
        wordsToTags.incrementCount(word, tag, 1.0);
        tagToWords.incrementCount(tag, word, 1.0);
        seenTagTrigrams.add(makeTrigramString(labeledLocalTrigramContext
            .getPreviousPreviousTag(), labeledLocalTrigramContext
            .getPreviousTag(), labeledLocalTrigramContext.getCurrentTag()));
        String bigram = makeBigramString(labeledLocalTrigramContext
            .getPreviousPreviousTag(), labeledLocalTrigramContext
            .getPreviousTag());
        prepreToTags.incrementCount(bigram, tag, 1.0);
        preToTags.incrementCount(labeledLocalTrigramContext.getPreviousTag(),
            tag, 1.0);
        tags.incrementCount(tag, 1.0);
        words.incrementCount(word, 1.0);
      }
      wordsToTags = Counters.conditionalNormalize(wordsToTags);
      tagToWords = Counters.conditionalNormalize(tagToWords);
      unknownWordTags = Counters.normalize(unknownWordTags);
      preToTags = Counters.conditionalNormalize(preToTags);
      prepreToTags = Counters.conditionalNormalize(prepreToTags);
      tags = Counters.normalize(tags);

      List<LabeledInstance<String, String>> list = new ArrayList<>();
      for (String word : words.keySet()) {
        if (words.getCount(word) < 5) {

          Counter<String> counter = wordsToTags.getCounter(word);
          if (counter.size() == 1) {
            String tag = counter.keySet().iterator().next();
            list.add(new LabeledInstance(tag, word));
          }
        }
      }
      ProbabilisticClassifierFactory<String, String> factory = new
          MaximumEntropyClassifier.Factory<String, String, String>(2.0, 50,
          new UNKExtractor());
      classifier = factory.trainClassifier(list);
    }

    public void validate(List<LabeledLocalTrigramContext>
                             labeledLocalTrigramContexts) {
  ...
  private static List<TaggedSentence> readTaggedSentences(String path,
    ...
    while ((line = reader.readLine()) != null) {
      if (line.equals("")) {
        taggedSentences.add(new TaggedSentence(new BoundedList<String>(words,
            START_WORD, STOP_WORD), new BoundedList<String>(tags, START_WORD,
            STOP_WORD)));
  ...
  private static void labelTestSet(POSTagger posTagger, List<TaggedSentence>
      testSentences, String path) throws Exception {
  ...
  private static void evaluateTagger(POSTagger posTagger,
                                     ...
                                     Set<String> trainingVocabulary, boolean
                                         verbose) {
    double numTags = 0.0;
      ...
      for (int position = 0; position < words.size() - 1; position++) {
        ...
        if (!trainingVocabulary.contains(word)) {
          if (guessedTag.equals(goldTag)) {
            ...
            System.out.println(word + " " + guessedTag + "| gold:" + goldTag);
          }
          numUnknownWords += 1.0;
        }
      }
      double scoreOfGoldTagging = posTagger.scoreTagging(taggedSentence);
      double scoreOfGuessedTagging = posTagger.scoreTagging(new
          TaggedSentence(words, guessedTags));
      if (scoreOfGoldTagging > scoreOfGuessedTagging) {
        numDecodingInversions++;
        if (verbose)
          System.out.println("WARNING: Decoder suboptimality detected.  Gold " +
              "" + "tagging has higher score than guessed tagging.");
      }
      if (verbose)
        System.out.println(alignedTaggings(words, goldTags, guessedTags,
            true) + "\n");
    }
  ...
  private static String alignedTaggings(List<String> words, List<String>
      goldTags, List<String> guessedTags, boolean suppressCorrectTags) {
  ...
  private static Set<String> extractVocabulary(List<TaggedSentence>
                                                   taggedSentences) {
  ...
  public static void main(String[] args) throws Exception {
    ...
    List<TaggedSentence> trainTaggedSentences = readTaggedSentences(basePath
        + "/en-wsj-train.pos", true);
    Set<String> trainingVocabulary = extractVocabulary(trainTaggedSentences);
    System.out.println("done.");
    System.out.print("Loading in-domain dev sentences...");
    List<TaggedSentence> devInTaggedSentences = readTaggedSentences(basePath
        + "/en-wsj-dev.pos", true);
    System.out.println("done.");
    System.out.print("Loading out-of-domain dev sentences...");
    List<TaggedSentence> devOutTaggedSentences = readTaggedSentences(basePath
        + "/en-web-weblogs-dev.pos", true);
    System.out.println("done.");
    ...
    evaluateTagger(posTagger, devOutTaggedSentences, trainingVocabulary,
        verbose);
    labelTestSet(posTagger, testSentences, basePath + "/../output.txt");
  }
}
