 ...
 * punctuation is NOT currently deleted properly (approximate hack), and other
...
public class EnglishPennTreebankParseEvaluator<L> {
	abstract static class AbstractEval<L> {
		...
		public void evaluate(Tree<L> guess, Tree<L> gold, PrintWriter pw) {
			...
			// guess.pennPrint(pw);
			// gold.pennPrint(pw);
			displayPRF(str + " [Current] ", correctSet.size(),
					guessedSet.size(), goldSet.size(), currentExact, 1, pw);

		}

		private void displayPRF(String prefixStr, int correct, int guessed,
		...
		public void display(boolean verbose, PrintWriter pw) {
			displayPRF(str + " [Average] ", correctEvents, guessedEvents,
					goldEvents, exact, total, pw);
		}
	}
